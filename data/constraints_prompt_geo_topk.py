...
class HOS_prompt_topk:
    def __init__(self, npy_file_path, output_filename, horizontal_n, top_k, include_agent_id=True, use_grid_id=False, is_interval=False, 
                 event_permute=False, feature_permute=False, more_frequent=False):
        ...
        ## Newly added
        self.top_k = top_k
        self.more_frequent = more_frequent
    ...
    def collect_grid_id_frequencies(self, grouped_data):
        grid_id_counter = Counter()
        ...
        for key, events in grouped_data.items():
            ...
            for event in events:
                ...
                grid_id_counter[grid_id] += 1

        return grid_id_counter

    def get_top_k_frequencies(self, grid_id_counter, top_k=10):
        most_frequent = dict(grid_id_counter.most_common(top_k))
        least_frequent = dict(sorted(grid_id_counter.items(), key=lambda item: item[1])[:top_k])
    ...
    def process_events(self):
        ...
        output_data = []
        grid_id_counter = self.collect_grid_id_frequencies(grouped_data)
        most_frequent, least_frequent = self.get_top_k_frequencies(grid_id_counter)
        ...
        for key, events in grouped_data.items():
            ...
            # sampled_events = random.sample(events, random.randint(1, len(events)))
            ...
            for event in events:
                ...
                if self.more_frequent:
                    if grid_id not in most_frequent:
                        continue
                else:
                    if grid_id not in least_frequent:
                        continue
            ...
            if not event_descriptions:
                continue
            num_events_to_select = random.randint(1, min(3,len(event_descriptions)))
            sampled_event_descriptions = random.sample(event_descriptions, num_events_to_select)
            output_data.append([f"{';'.join(sampled_event_descriptions)};"])
        
        if not self.more_frequent:
            replication_factor = 800 
            output_data = output_data * replication_factor
...
if __name__ == '__main__':
    ...
    #################### Constraint Geolife WO agent id, W grid id, W 15min interval representation, W event permute W departure####################
    npy_file_path = ''
    output_filename = ''
    horizontal_n = 200
    top_k = 40
    hos_sampler = HOS_prompt_topk(npy_file_path, output_filename, horizontal_n, top_k, include_agent_id=False, use_grid_id=True, 
                             is_interval=True, event_permute=True, feature_permute=False, more_frequent=False)
